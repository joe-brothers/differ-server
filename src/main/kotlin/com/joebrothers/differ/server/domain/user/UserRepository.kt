...
class UserRepositoryImpl : UserRepository {
    override suspend fun existsByUsername(username: String): Boolean = UsersTable
        .select(UsersTable.id)
        .where { UsersTable.name eq username }
        .limit(1)
        .count() > 0

    override suspend fun create(username: String, password: String, email: String?): UUID = UsersTable.insertAndGetId {
    ...
    override suspend fun selectByUsername(username: String): UserEntity? = UsersTable
        .selectAll()
        .where { UsersTable.name eq username }
        .limit(1)
